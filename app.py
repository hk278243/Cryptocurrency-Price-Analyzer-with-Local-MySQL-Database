...
from passwords import _mysql_passwords
...
app.config['MYSQL_CURSORLASS'] = 'DictCursor'
...
def register():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if isnewuser(username): # check if new user
            password = sha256_crypt.encrypt(form.data)
...
@app.route("/tranction", methods = ['GET', 'POST'])
@is_logged_in
def transaction():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('transction')) 
...
def buy():
    ...
    if request.method == 'POST':
        try:
            send_money("BANK", session.get('username'), form.username.data, form.amount.data)
